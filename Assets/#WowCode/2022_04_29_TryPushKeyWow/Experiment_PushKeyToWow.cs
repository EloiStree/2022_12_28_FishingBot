...
public class Experiment_PushKeyToWow : MonoBehaviour
{
    public string m_nameWindowToPushOn= "World of Warcraft";
    public KeyCode m_unityKeytoPress = KeyCode.Space;
    public User32PostMessageKeyEnum m_toSend = User32PostMessageKeyEnum.VK_SPACE;

    public void Update()
    {
        if (Input.GetKeyDown(m_unityKeytoPress))
        {
            SendKeyMessageToWindows.Utility.FindWindowParentProcess(m_nameWindowToPushOn, out
    IntPtrWrapGet found);
            SendKeyMessageToWindows.SendKeyDown(m_toSend, found, true);
        }
        if (Input.GetKeyDown(m_unityKeytoPress))
        {
            SendKeyMessageToWindows.Utility.FindWindowParentProcess(m_nameWindowToPushOn, out
    IntPtrWrapGet found);
            SendKeyMessageToWindows.SendKeyUp(m_toSend, found, true);
        }
    }
}
