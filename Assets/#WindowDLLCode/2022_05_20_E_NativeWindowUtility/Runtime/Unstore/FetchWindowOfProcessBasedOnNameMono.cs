...
public class DeductedInfoOfWindowSize
{
    ...
    private void GetRightToLeftToAbsolute(float percent, out int x)
    {
        Eloi.E_CodeTag.QualityAssurance.RequestTestingInAsSoonAsPossible();
        x = (int)(m_givenWithoutBorder.m_borderLeft + m_innerWidth * (1f-percent) );
    }
    ...
    public void GetAbsoluteFromRelativePixelRight2Left(int pixelRight2LeftRelative, out int x)
    {
        float percent = pixelRight2LeftRelative / (float)m_innerWidth;
        GetRightToLeftToAbsolute(percent, out x);
    }
